...
keep UnitID Institution AD AE AF AG AH AI AJ AK AL T
destring AD AE AF AG AH AI AJ AK AL T, replace 
rename T TOT2023
rename AG WHITE2023
rename AD NONRES2023
rename AL UNKNOWN2023
rename Institution Name
gen DOMMIN2023 = AE + AF + AH + AI + AJ + AK 
...
import excel "$input/Race from IPEDS.xlsx", sheet("Race Data") cellrange(A2:CY1853) firstrow clear
drop RENAMEDBYIRAP
keep if inlist(UnitID,$comparison) 
...
use "$temp/all_students_current.dta", clear
merge 1:1 UnitID using "$temp/all_students_historical.dta"
*assert _merge == 3
drop _merge
...
graph twoway (scatter perc year if UnitID!= 238333) (scatter perc year if UnitID== 238333)(connected med year, connect(direct)), ///
	...
	subtitle("All students, 2014-203", color(black)) ///
	scheme(s2color) ///
	xlabel(2014(1) 2023) ///
...
graph twoway (scatter perc year if UnitID!= 238333) (scatter perc year if UnitID== 238333)(connected med year, connect(direct)), ///
	...
	subtitle("All students, 2014-2024", color(black)) ///
	scheme(s2color) ///
	xlabel(2014(1) 2023) ///
    note("Source: IPEDS; Common Data Set") ///
	graphregion(fcolor(white)) 
