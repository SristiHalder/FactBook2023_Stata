...
import excel using "$input/2023-24 - Faculty Salaries as percent of ACM median - AAUP Compilation.xlsx", sheet("All") firstrow cellrange(A1)

*** Keep the data of comparison schools only 
keep if inlist(UnitID,$acm) 
keep UnitID InstitutionName fullwage* assocwage* assistwage*
...
graph twoway (scatter real_salary Year if UnitID!= 238333) (scatter real_salary Year if UnitID== 238333)(connected med Year, connect(direct)), ///
	...
	xtitle("") ///
	xlabel(2014(1) 2023) ///
	note("Source: ACM compilation of AAC&U Faculty Compensation Survey") ///
...
graph twoway (scatter real_salary Year if UnitID!= 238333) (scatter real_salary Year if UnitID== 238333)(connected med Year, connect(direct)), ///
	...
	xtitle("") ///
	xlabel(2014(1) 2023) ///
	note("Note: Inflation adjustment using Bureau of Labor Statistics: CPI-All Urban Consumers in 2023" "Source: ACM compilation of AAC&U Faculty Compensation Survey") ///
	graphregion(fcolor(white)) ///
	scheme(s2color) ///
...
*** Keep the data of comparison schools only 
keep if inlist(UnitID,$acm) 
keep UnitID InstitutionName fullwage2023 assocwage2023 assistwage2023
...
graph hbar (asis) assocwage2023, over(Name, sort(assocwage2023)) ///
	...
	yscale (extend) ///
	ytitle("Salary ($)") ///
	ylabel(0(20000)120000, format(%12.0fc)) ///
    bar(1, color("0 56 107") fintensity(100)) 
