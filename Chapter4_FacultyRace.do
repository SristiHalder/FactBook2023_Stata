...
keep UnitID Institution D E H

rename D prof_FT_total_current
rename E prof_FT_dommin_current
rename H prof_FT_intl_current

save "$temp/faculty_race_current.dta", replace
...
keep if inlist(UnitID,$comparison) 
keep domestic* unknownprof* whiteprof* interprof* UnitID Name profmen* profwomen* proftotal*

label var Domesticminority2020 "Domestic Minority"  
label var International2020 "International"
...
***************************************************************************************
* Graph: Faculty Race/Ethnicity Distriction, current year, Beloit and comparison
***************************************************************************************
...
********************************************************************
* Graph: Faculty Domestic White, over time, Beloit and comparison
********************************************************************
...
graph twoway (scatter White year if UnitID!= 238333) (scatter White year if UnitID== 238333) (connected med year, connect(direct)), ///
	...
	scheme(s2color) ///
    note("Note: 2017 domestic white also includes domestic faculty of unknown race/ethnicity""Source: IPEDS; Common Data Set") ///
	xlabel(2013 2014 2015 2016 2017 2018 2019 2020) ///
...
********************************************************************
* Graph: Faculty Domestic Minority, current year, Beloit and comparison
********************************************************************


*Domestic Minority
graph hbar (asis) Domesticminority2020, over (Name, sort (1)) ///
  blabel(bar, pos(inside) color(white) size(2) format(%9.0f)) ///
  title ("Percentage of Faculty who are Domestic Minority", color(black)) ///
  subtitle("Full-Time, 2020", color(black)) ///
  ytitle ("Percentage") ///
  scheme(colorblind) ///
  yscale(extend) ///
  note("Note: Data unavailable for Allegheny, Ursinus, Centre, Rhodes and Ohio Wesleyan" "Source: Common Data Set" ) ///
  bar(1, color("0 56 107") fintensity(100))	///
  graphregion(color(white))
  	 
graph export "$png/Faculty_domestic_minority_2020.png", as(png) replace
*graph export "$wmf/Faculty_domestic_minority_2019.wmf", as(wmf) replace
graph export "$final/Faculty_domestic_minority_2020.pdf", as(pdf) replace 
	 


********************************************************************
* Graph: Faculty Domestic Minority, over time, Beloit and comparison
********************************************************************
...
graph hbar (asis) International2023, over (Name, sort (1)) ///
  ...
  scheme(colorblind) ///
  yscale(extend) ///
  note("Note: International is defined as non-resident aliens. Green card holders and ""naturalized U.S.citizens are not included here.""Note: Data unavailable for Data unavailable for Allegheny, Ursinus, Centre," "Rhodes and Ohio Wesleyan" "Source: Common Data Set" ) ///
  bar(1, color("0 56 107") fintensity(100))	///
  graphregion(color(white))
  
  
graph export "$png/Faculty_international_2020.png", as(png) replace
graph export "$wmf/Faculty_international_2020.wmf", as(wmf) replace 
graph export "$final/Faculty_international_2020.pdf", as(pdf) replace



********************************************************************
* Graph: Faculty International, over time, Beloit and comparison
********************************************************************
...
graph twoway (scatter International year if UnitID!= 238333) (scatter International year if UnitID== 238333)(connected med year, connect(direct)), ///
	...
	subtitle("Full-Time Faculty", color(black)) ///
	scheme(s2color) ///
    note("Note: International is defined as non-resident aliens. Green card holders and naturalized U.S.""citizens are not included here.""Source: IPEDS; Common Data Set") ///
	xlabel(2013 2014 2015 2016 2017 2018 2019 2020) ///
	ylabel(0(5) 15) ///
	graphregion(fcolor(white)) 
